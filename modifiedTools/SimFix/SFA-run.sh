...
if [ ! -d "SFA-terminalLog/" ]; then
	mkdir SFA-terminalLog/
fi

terminalLog=SFA-terminalLog/
...
do
	...
	do
			...
			java -jar SFA-simfix.jar --proj_home=/home/dehengyang/SimFix/buggyprograms --proj_name=${project[i]} --bug_id=${j} > $terminalLog/${project[i]}_${j}_${k}.txt
