...
*/
		...
		for (int i = gz.getSuspiciousStatements().size() - 1; i >= 0; i--) {
			Statement statement = gz.getSuspiciousStatements().get(i);
			...
			int ef=executedAndFailedCount;
			int ep=executedAndPassedCount;
			int np=sum[0] - sum[1] - executedAndPassedCount;
			int nf=sum[1] - executedAndFailedCount;

			double susp=ef / ((double) (ef + ep + nf));
			
			/*double susp= 0;
			if(ef+nf == 0) {
			    susp= 0;
			}else{
				susp= (ef / ((double) (ef + nf))) / ((ef / ((double)(ef + nf))) + (ep/ ((double) ep + np))) ;
			}*/


			//double susp= ef-ep/(double)(ep+np+1);
			//if(susp<0){
			//	System.out.println("deheng-the susp is less than zero(0) :" + susp + " ef:"+ef
			//		+"ep:"+ep+"np:"+np+"nf:"+nf); 
			//	susp=0;
			//}
			statement.setSuspiciousness(susp);
/*
