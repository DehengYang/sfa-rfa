...
public class ProgramVariantFactory {
	...
	public List<ProgramVariant> createInitialPopulation(List<SuspiciousCode> suspiciousList, int maxNumberInstances,
		...
		System.out.println("deheng maxNumberInstances: "+maxNumberInstances);
		for (int ins = 0; ins < suspiciousList.size(); ins++){
			SuspiciousCode sc = suspiciousList.get(ins);
			//System.out.println("deheng-" + ins +"th SuspiciousCode info: className is: "+sc.getClassName() + " getMethodName: " +sc.getMethodName() + " getLineNumber: " + sc.getLineNumber());
			System.out.println("deheng-" + ins +"th SuspiciousCode info:"+sc.toString());
		}
	...
	/**  
	 * A Program instances is created from the list of suspicious. For each //@SuppressWarnings("rawtypes")
	...
	private ProgramVariant createProgramInstance(List<SuspiciousCode> suspiciousList, int idProgramInstance) {
		...
		if (!suspiciousList.isEmpty()) {
			for (SuspiciousCode suspiciousCode : suspiciousList) {
				...
				// is possible)
				List<ModificationPoint> modifPoints = createModificationPoints(suspiciousCode, progInstance); //deheng ModificationPoint
				//log.info("modifPoints size:"+modifPoints.size());
				//System.out.println("modifPoints size:"+modifPoints.size());
				if (modifPoints != null){
					if(modifPoints.size() != 0){
						progInstance.addModificationPoints(modifPoints);
						System.out.println("modifPoints size:"+modifPoints.size() + " progInstance.getModificationPoints().size() : " + progInstance.getModificationPoints().size() +"\nkali-susp:"+modifPoints.get(0).getSuspicious());
					}
				}else {
			...
			List<ModificationPoint> pointsFromAllStatements = createModificationPoints(progInstance); //deheng ModificationPoint
	...
	private List<ModificationPoint> createModificationPoints(ProgramVariant progInstance) {  //deheng ModificationPoint

		List<ModificationPoint> suspGen = new ArrayList<>();
		...
		for (CtClass ctclasspointed : classesFromModel) {
			...
			if(allTest.contains(testn) ){
				...
				log.info("ModifPoints creation: Ignoring test case "+testn);
				continue;
			}
			...
			System.out.println("extractedElements size: "+extractedElements.size());
			...
			for (CtElement suspiciousElement : extractedElements) {
				...
				ModificationPoint point = new ModificationPoint();
	 ...
	 * @return
	 */
	private List<ModificationPoint> createModificationPoints(SuspiciousCode suspiciousCode, ProgramVariant progInstance) {

		List<ModificationPoint> suspGen = new ArrayList<ModificationPoint>(); //deheng ModificationPoint
		if(suspGen == null){
			log.info("null suspGen");
		}else{
			log.info("not null suspGen");
		}
		...
		int id = 0;
		System.out.println("filteredTypeByLine size: "+filteredTypeByLine.size());
		for (CtElement ctElement : filteredTypeByLine) {
			...
			System.out.println("suspGen size: "+suspGen.size());
